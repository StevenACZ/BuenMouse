...
ICON_PATH="${ICON_PATH:-Resources/Assets.xcassets/AppIcon.appiconset/BuenMouse 512.png}"
OUTPUT_DMG="${OUTPUT_DMG:-}"
NOTARY_PROFILE="${BUENMOUSE_NOTARY_PROFILE:-${NOTARY_PROFILE:-notarytool-dmg}}"

usage() {
  echo "Usage: $0 [--output <path>]"
  echo
  echo "Environment:"
  echo "  BUENMOUSE_SIGN_IDENTITY    Developer ID Application identity. Defaults to the first local Developer ID Application identity."
  echo "  BUENMOUSE_DEVELOPMENT_TEAM Development team. Defaults to the signing certificate OU."
  echo "  BUENMOUSE_NOTARY_PROFILE   notarytool keychain profile. Defaults to notarytool-dmg."
  echo "  DERIVED_DATA               Xcode DerivedData path. Defaults to build/notarized-release."
  echo "  OUTPUT_DMG                 Output DMG path. Defaults to ~/Downloads/BuenMouse-v<version>.dmg."
}
...
SIGN_IDENTITY="${BUENMOUSE_SIGN_IDENTITY:-${CODE_SIGN_IDENTITY:-}}"
...
if [[ -z "$SIGN_IDENTITY" ]]; then
  ...
  echo "Install the certificate with its private key, or set BUENMOUSE_SIGN_IDENTITY." >&2
  exit 65
fi

TEAM_ID="${BUENMOUSE_DEVELOPMENT_TEAM:-${DEVELOPMENT_TEAM:-}}"
if [[ -z "$TEAM_ID" ]]; then
...
if [[ -z "$TEAM_ID" ]]; then
  ...
  echo "Set BUENMOUSE_DEVELOPMENT_TEAM or DEVELOPMENT_TEAM and retry." >&2
  exit 65
fi
...
MOUNT_POINT="$(mktemp -d "${TMPDIR:-/tmp}/buenmouse-dmg.XXXXXX")"
