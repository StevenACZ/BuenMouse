...
- Quiet checks keep running while an update card is waiting and after a failed update, so a newer version shows up without restarting BuenMouse. The card only changes when a newer version is actually found, a failed update keeps its Retry, and nothing is ever installed without "Install now".

## [3.3.1] - 2026-09-18

### Changed

- New versions show up on their own within minutes: BuenMouse checks quietly when you open the menu bar panel, after waking and every 30 minutes (only while automatic checks are on).
...
- Update card in the menu bar panel: new versions now appear as a card with
  live download progress and a "Ready to install" step that offers Install now
  or Later. Later keeps the update ready and installs it the next time you quit
  BuenMouse, and the About window shows the pending update until then.
...
- Retry after a failed update downloads it again and still asks before installing.
